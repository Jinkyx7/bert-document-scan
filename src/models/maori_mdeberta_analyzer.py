"""
Māori wellbeing analyzer using MoritzLaurer/mDeBERTa-v3-base-mnli-xnli zero-shot classification.

This module identifies sentences related to Māori wellbeing using mDeBERTa, a state-of-the-art
multilingual model with excellent cross-lingual transfer capabilities.
"""
...
class MaoriMDeBERTaAnalyzer:
    """
    Analyzer for identifying Māori wellbeing content using mDeBERTa multilingual model.

    Uses a hybrid approach:
    1. Keyword matching with Māori health lexicon (frameworks, practices, terminology)
    2. Zero-shot classification using MoritzLaurer/mDeBERTa-v3-base-mnli-xnli (multilingual)
    3. Combined scoring to improve accuracy

    Key advantages of mDeBERTa:
    - State-of-the-art multilingual understanding
    - Strong zero-shot classification performance
    - Better handling of complex semantic relationships
    - Efficient inference (base model size)
    """

    def __init__(self, threshold: float = 0.7, batch_size: int = 8):
        """
        Initialize the Māori wellbeing analyzer with mDeBERTa.
    ...
    def _zero_shot_score(self, sentence: str) -> float:
        """Calculate zero-shot classification score using mDeBERTa."""
        try:
    ...
    def analyze_report(self, sentences_data: List[Dict], report_name: str, output_dir: str) -> Dict[str, Any]:
        ...
        print(f"Analyzing {len(df)} sentences for Māori wellbeing content (mDeBERTa)...")
        ...
        # Save all results
        all_csv = os.path.join(output_dir, f"maori_mdeberta_results_all_{report_name}.csv")
        ...
        hits_csv = os.path.join(output_dir, f"maori_mdeberta_results_hits_{report_name}.csv")
        ...
        return {
            ...
            "maori_mdeberta_candidates": int(len(hits)),
            "all_csv": all_csv,
            "hits_csv": hits_csv,
