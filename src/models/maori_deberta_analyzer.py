"""
Māori wellbeing analyzer using microsoft/deberta-v3-base zero-shot classification.

This module identifies sentences related to Māori wellbeing using DeBERTa v3, a fast and
accurate English-focused model with strong zero-shot classification performance.
"""
...
class MaoriDeBERTaAnalyzer:
    """
    Analyzer for identifying Māori wellbeing content using DeBERTa v3 base model.

    Uses a hybrid approach:
    1. Keyword matching with Māori health lexicon (frameworks, practices, terminology)
    2. Zero-shot classification using microsoft/deberta-v3-base
    3. Combined scoring to improve accuracy

    Key advantages of DeBERTa v3:
    - Fast inference (base model size)
    - Strong zero-shot classification accuracy
    - Efficient for large-scale processing
    - Good at understanding English text with Māori terminology
    """

    def __init__(self, threshold: float = 0.7, batch_size: int = 16):
        """
        Initialize the Māori wellbeing analyzer with DeBERTa v3.

        Args:
            threshold: Hybrid score threshold for filtering hits (0.0-1.0)
            batch_size: Number of sentences to process simultaneously
                       (DeBERTa is faster, can use larger batches)
        """
    ...
    def _zero_shot_score(self, sentence: str) -> float:
        """Calculate zero-shot classification score using DeBERTa v3."""
        try:
    ...
    def analyze_report(self, sentences_data: List[Dict], report_name: str, output_dir: str) -> Dict[str, Any]:
        ...
        print(f"Analyzing {len(df)} sentences for Māori wellbeing content (DeBERTa v3)...")
        ...
        # Save all results
        all_csv = os.path.join(output_dir, f"maori_deberta_results_all_{report_name}.csv")
        ...
        hits_csv = os.path.join(output_dir, f"maori_deberta_results_hits_{report_name}.csv")
        ...
        return {
            ...
            "maori_deberta_candidates": int(len(hits)),
            "all_csv": all_csv,
            "hits_csv": hits_csv,
